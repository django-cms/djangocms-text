...
class PluginActionsTestCase(TestFixture, BaseTestCase):
    ...
    def _add_child_plugin(self, text_plugin, plugin_type="PicturePlugin", data_suffix=None):
        ...
        basic_plugins = {
            "LinkPlugin": {
                "name": name,
                "link": {"external_link": "https://www.django-cms.org"},
            },
        ...
        if DJANGO_CMS4:
            ...
            if DJANGOCMS_VERSIONING:
                ...
                edit_endpoint = get_object_edit_url(
                    PageContent._original_manager.filter(page=simple_page, language="en", version__state=DRAFT).first()
                )
            else:
...
class DjangoCMSTranslationsIntegrationTestCase(BaseTestCase):
    ...
    def test_textfield_with_children(self):
        ...
        expected = parent_body.replace("></cms-plugin>", ">CLICK ON LINK1</cms-plugin>", 1)
    ...
    def test_textfield_with_multiple_children(self):
        ...
        expected = parent_body.replace("></cms-plugin>", ">CLICK ON LINK1</cms-plugin>", 1).replace(
            "></cms-plugin>", ">CLICK ON LINK2</cms-plugin>", 1
        )
        self.assertEqual(result, expected)
